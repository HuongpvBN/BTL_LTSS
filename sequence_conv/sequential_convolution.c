...
int main(int argc, char **argv)
{
    ...
    if (!h)
    {
        fprintf(stderr, "[SEQ] Error allocating filter rows\n");
        return EXIT_FAILURE;
    }
    for (i = 0; i < 3; i++)
    {
        ...
        if (!h[i])
        {
            fprintf(stderr, "[SEQ] Error allocating filter cols\n"); /* cleanup */
            return EXIT_FAILURE;
        }
    }
    ...
    for (t = 0; t < loops; ++t)
    {
        ...
        for (i = 1; i <= height; ++i)
        { // Hàng logic 1 đến height
            for (j = 1; j <= width; ++j)
            { // Cột logic 1 đến width
                if (imageType == GREY)
                {
    ...
    if (!success)
    {
        fprintf(stderr, "[SEQ] Error writing PNG file.\n");
    }
    else
    {
        printf("[SEQ] Successfully wrote PNG file.\n");
    }
